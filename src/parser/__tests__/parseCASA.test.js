...
import { parseCASA } from '../parseCASA.js';

test('assigned', () => {
  const text = readFileSync(
    join(import.meta.dirname, './data/casaBlockComment.txt'),
    'utf8',
  );
...
test('Tougaard background', () => {
  ...
  expect(casa.regions[0]).toStrictEqual({
    ...
    background: {
      ...
      rawParameters: '1193.9028 1205.0344 0.278 2 0 0 786.70222 -450 0 0',
    },
  });
});

test('Shirley background', () => {
  let casa = parseCASA(
...
test('issue #9', () => {
  ...
  expect(casa.components[0].shape.kind).toBe('lorentzianAsymmetric');
});
